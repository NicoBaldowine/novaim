...
export function SignalField({ mode = "signals", interactive = false, paused: externallyPaused = false, animatePalette = false, neural = false, pointScale = 1, gridDensity = 1, fixedPointSize, rotationRate = .085, ink, ariaLabel }: { ariaLabel?: string; rotationRate?: number; fixedPointSize?: number; gridDensity?: number; pointScale?: number; neural?: boolean; ink?: string; animatePalette?: boolean; mode?: SignalMode; interactive?: boolean; paused?: boolean }) {
  ...
  useEffect(() => {
    ...
    function draw(now: number) {
      ...
      const cols = Math.round(29 * gridDensity), rows = Math.round((interactive ? 29 : 25) * gridDensity);
      ...
      if (mode === "positioning") {
        ...
        const count = 3200;
        const rotation = flowTime * rotationRate;
        const tilt = .22 + Math.sin(time * .12) * .045;
        const cr = Math.cos(rotation), sr = Math.sin(rotation);
        const ct = Math.cos(tilt), st = Math.sin(tilt);
        const renderedScale = fixedPointSize && canvasElement.clientWidth
          ? Math.max(.35, canvasElement.getBoundingClientRect().width / canvasElement.clientWidth)
          : 1;
        for (let i = 0; i < count; i++) {
          ...
          const pointerX = (focusX - .5) * 2;
          const pointerY = (focusY - .5) * 2;
          const pointerDx = rx - pointerX;
          const pointerDy = ry - pointerY;
          const pointerDistance = Math.hypot(pointerDx, pointerDy);
          const response = influence * Math.exp(-(pointerDistance ** 2) / .22);
          const dispersion = response * .085;
          const directionX = pointerDx / Math.max(pointerDistance, .08);
          const directionY = pointerDy / Math.max(pointerDistance, .08);
          const scatterX = Math.sin(i * 17.17) * dispersion * .18;
          const scatterY = Math.cos(i * 11.73) * dispersion * .18;
          let activation = 0;
          if (neural) {
          ...
          const pointRadius = fixedPointSize
            ? (fixedPointSize / renderedScale) * (1 + edge * .12 + wave * .08 + activation * .48)
            : pointScale * size * (.0019 + .00035 * edge + .0002 * wave + activation * .001);
          dot(width / 2 + (rx + directionX * dispersion + scatterX) * densityScale * radius,
            height / 2 + (ry + directionY * dispersion + scatterY) * densityScale * radius,
            energy, pointRadius);
        }
      }
      for (let y = 0; mode !== "positioning" && mode !== "principles" && y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          dot(px, py, energy, gap * (.085 + energy * .11) * pointScale);
        }
      }
  ...
  }, [mode, interactive, externallyPaused, animatePalette, ink, neural, pointScale, gridDensity, fixedPointSize, rotationRate]);
  const graphic = <canvas ref={canvas} className="signal-canvas" aria-hidden="true" />;
  if (!interactive) return graphic;
  return <button type="button" className="signal-interaction" aria-label={ariaLabel ?? prompts[mode]} onPointerMove={event => {
